...
3. All content of the data directory, except for the `mmpcoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `mmpcoin.conf`       | User-defined configuration settings for `mmpcoind` or `mmpcoin-qt`; can be specified by `-conf` option
`./`               | `mmpcoind.pid`        | Stores the process ID (PID) of `mmpcoind` or `mmpcoin-qt` while running; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `mmpcoind` or `mmpcoin-qt`
...
`mmpcoin-qt` uses the [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).
