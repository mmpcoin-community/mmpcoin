...
**Optional dependencies** :
```bash
# Qt (required for mmpcoin-qt GUI)
...
According to installed dependencies, the following steps will compile `mmpcoind`, `dogecoin-cli` and `mmpcoin-qt`.
...
#### Enable Qt GUI
Create `mmpcoin-qt`, the core wallet GUI.
```bash
./configure --with-gui
```
