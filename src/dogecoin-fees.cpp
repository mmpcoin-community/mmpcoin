...
CFeeRate GetDogecoinFeeRate(int priority)
{
    ...
    {
    case SUCH_EXPENSIVE:
        return CFeeRate(COIN / 100 * 258); // 2.58 MMP, but very carefully avoiding floating point maths
    case MANY_GENEROUS:
...
CAmount GetDogecoinMinRelayFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree)
{
    ...
    nMinFee += GetmmpcoindustFee(tx.vout, nDustLimit);
...
CAmount GetmmpcoindustFee(const std::vector<CTxOut> &vout, const CAmount dustLimit) {
    CAmount nFee = 0;
